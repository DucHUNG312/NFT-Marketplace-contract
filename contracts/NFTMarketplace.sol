    ...
    IERC1155ReceiverUpgradeable
{
    ...
    function updateListing(
        ...
        bool isAution = targetListing.listingType == ListingType.Auction;
    ...
    function handleBid(Listing memory _targetListing, Offer memory _incomingBid)
        internal
    {
        Offer memory currentWiningBid = winningBid[_targetListing.listingId];
        uint256 currentOfferAmount = currentWiningBid.pricePerToken *
            currentWiningBid.quantityWanted;
        ...
        if (currentWiningBid.offeror != address(0) && currentOfferAmount > 0) {
            CurrencyTransferLib.transferCurrencyWithWrapper(
                _targetListing.currency,
                address(this),
                currentWiningBid.offeror,
                currentOfferAmount,
                _nativeTokenWrapper
            );
        }
    ...
    function payout(
        ...
        // pay platform fee for marketplace
        CurrencyTransferLib.transferCurrencyWithWrapper(
            _currencyToUse,
            _payer,
    ...
    function validateDirectListingSale(
        ...
        if (_currenncy == CurrencyTransferLib.NATIVE_TOKEN) {
    ...
    // get the quantity of token (for ERC721 it always is 1)
    ...
    // get token type of the asset contract
